...
//    var elements = contentDiv.getElements(".editsection");
        ...
        while (!pos.is("h2")) {
            ...
            // prevent going to section of same listing
            if (pos.is("h3"))
                break;
	...
	if(newListing && !pos.is("ul"))
		lastPosition.after($(div.firstChild));
	else
        	pos.append($(div.firstChild).find("li"));
...
function doAjaxCall(serverFunction, params, cb, animationControl) {
    ...
    doAjaxRequest(serverFunction, params, cb);
...
function cMessageHandler (locatorElement){
    this.initialize = function (locatorElement) {
    ...
    this.createMessageNode = function (status, message) {
    ...
    this.initialize(locatorElement);
};


//var cEditMessageHandler = cMessageHandler.extend({
function cEditMessageHandler(locatorElement) {
    ...
    this.displayMessage = function (status, message) {
        ...
        $(node).before(this.locatorElement);
    };
    this.createMessageNode = function (status, message) {
        var text = message + "&nbsp;<span class='dismiss'>dismiss</span>";
...
function cAddMessageHandler(locatorElement) {
    //Extends: cMessageHandler,
    this.initialize = function (locatorElement) {
    //console.log(locatorElement);
        //this.parent(locatorElement);
        this.locElement = locatorElement;
    };
    this.displayMessage = function (status, message) {
        var node = this.createMessageNode(status, message);
        $(node).after(this.locatorElement);
    };
    this.createMessageNode = function (status, message) {
        var text = message + "&nbsp;<span class='dismiss'>dismiss</span>";
...
function cListingEditForm(title, tagname, listing, fields, article){
    this.initialize = function (title, tagname, listing, fields, article) {
    ...
    this.positionAtElement = function (element) {
        //console.log($(element));
        //console.log(element);
        //console.log(element.id);
        //return;
        var coords = $(element).position();
        this.form.style.top = coords.top + 190 + "px";
        this.form.style.left = coords.left + 100 + "px";
    ...
    this.clearSaveHandler = function () {
        var saveButton = document.getElementById("button-save");
        saveButton.onclick = null;
    };
    this.getAnimationControl = function () {
    ...
    this.createForm = function (title, tagname, listing, fields, article) {
    ...
    this.addClear = function (parent) {
        br = document.createElement('br');
        br.setAttribute('clear', 'all');
        parent.appendChild(br);
    };
    this.listingTextInput = function (name, value, options) {
    ...
    this.listingTextArea = function (name, value, options) {
    ...
    this.listingInput = function (name, element) {
        var div = document.createElement('div');
    ...
    this.listingHidden = function (name, value) {
        var input = document.createElement('input');
    ...
    this.makeLinkButton = function (id, text, onclick) {
...
//    a.appendChild(document.createTextNode(text));
	...
	this.initialize(title, tagname, listing, fields, article);
};

function cSaveAnimation(targetID){
